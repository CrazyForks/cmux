...
/// A manually-advanced virtual clock: time only moves when the test calls
/// ``advance(by:)``, so time-window behavior is exercised without real
/// waiting. Sleeps are cancellation-responsive (resuming with
/// `CancellationError`), matching the contract the scrolled-up hysteresis
/// relies on to drop a revoked flip. Duplicated from
/// `CmuxAuthRuntimeTests/ManualTestClock.swift` (test targets cannot import
